...
sensors_event_t humidity, temp; // Declare humidity and temp
...
void setup() {
  ...
  WiFiManager wm;
  bool res = wm.autoConnect("Group1RealPeach"); // Anonymous AP
  if (!res) {
...
void loop() {
  ...
  switch (state) {
    case SEND_AND_CHECK:
      ...
      temperature_sensor = temp.temperature;
      ...
      if (mqtt.connected()) {
        ...
        doc_pub["temperature"] = temperature_sensor;
